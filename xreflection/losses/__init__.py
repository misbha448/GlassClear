...
_model_modules = [importlib.import_module(f'xreflection.losses.{file_name}') for file_name in loss_filenames]
...
def build_loss(opt):
    ...
    rank_zero_info(f'Loss [{loss.__class__.__name__}] is created.')
    return loss