...
class Decoder(nn.Module):
    def __init__(self, depth=[2,2,2,2], dim=[112, 72, 40, 24], block_type = None, kernel_size = 3) -> None:
        ...
        self.projback_shit = nn.Sequential(
            nn.Conv2d(
                in_channels=dim[-1],
                out_channels=2 ** 2 * 3 , kernel_size=1),
            nn.PixelShuffle(2)
        )
        self.projback_shit_2 = nn.Sequential(
            nn.Conv2d(
                in_channels=dim[-1],
                out_channels=2 ** 2 * 3, kernel_size=1),
            nn.PixelShuffle(2)
    ...
    def forward(self, c3, c2, c1, c0):
        #c0 2 72 112 112 c1 2 144 56 56 c2 2 288 28 28 c3 2 576 14 14
        c0_clean, c0_ref = c0, c0 #torch.split(c0, c0.shape[-3] // 2, -3)
        c1_clean, c1_ref = c1, c1 #torch.split(c1, c1.shape[-3] // 2, -3)
        c2_clean, c2_ref = c2, c2 #torch.split(c2, c2.shape[-3] // 2, -3)
        c3_clean, c3_ref = c3, c3 #torch.split(c3, c3.shape[-3] // 2, -3)
        ...
        x_clean = self.projback_shit(x_clean)
        ...
        x_ref = self.projback_shit_2(x_ref)
        
        # reflection=self.pixelshuffle(c0[:,:12, ...])
        # clean=self.pixelshuffle(c0[:,60:72,...])
