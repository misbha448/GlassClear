...
def paired_data_transforms(img_1, img_2, transform_size=320, unaligned_transforms=False):
    ...
    target_size = int(random.randint(transform_size, transform_size + 256) / 2.) * 2
    ow, oh = img_1.size
    if ow >= oh:
...
class DSITFusionDataset(BaseDataset):
    def __init__(self, opt):
        super(DSITFusionDataset, self).__init__()
